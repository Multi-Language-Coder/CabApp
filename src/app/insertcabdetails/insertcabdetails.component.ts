...
export class InsertcabdetailsComponent {
  ...
  constructor(private googleMapsService: GoogleMapsService, private http: HttpClient, private route: ActivatedRoute) {
    ...
    this.route.params.subscribe(params => {
      this.userrequested = params['username']
  ...
  insertData() {
    ...
    this.http.post<number>("http://localhost:8080/idByTime", { time: this.time.value }).subscribe((val) => {
      this.id = val++;
      const cabdata = {
        cabid: this.id,
        fromLocation: (document.getElementById("FromLocationInput") as HTMLInputElement).value,
        toLocation: (document.getElementById("ToLocationInput") as HTMLInputElement).value,
